        ...
        const fetchBookings = async () => {
            try {
                ...
                for (let i = 0; i < result.bookings.length; i++) {
                    ...
                    const response = await fetch(`http://127.0.0.1:8000/v1/bookings/${bookingId}/tickets` , {
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Token ${localStorage.getItem("token")}`
                    }});
...
<div className="p-6 bg-gray-900 font-sans text-gray-200">
    <h1 className="text-2xl font-bold text-center text-gray-100 mb-6">Your Bookings</h1>
                    ...
                    {booking.tickets.map((ticket) => (
                        ...
                        >
                            <p className="text-gray-300">
                                <span className="font-medium text-gray-100">Movie:</span> {ticket.showtime.title}
                            </p>
                            <p className="text-gray-300">
                                <span className="font-medium text-gray-100">Time:</span> {ticket.showtime.time}
                            </p>
                            <p className="text-gray-300">
                                <span className="font-medium text-gray-100">Type:</span> {ticket.ticket_type.name}
                            </p>
                            <p className="text-gray-300">
                                <span className="font-medium text-gray-100">Seat:</span> {ticket.seat_number}
                            </p>
                        </li>
                    ))}
                </ul>
